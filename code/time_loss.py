    ...
    def calucate_gain_loss(self, x:float, y:float, timestamp):
        def calc_distance(self, current_x, current_y, current_timestamp):
            ...
            if points == '1-2':
                ...
                timestamp_inter = (nearest_point.timestmp - second_nearest_point.timestamp) * alfa
                absolute_delta = (second_nearest_point.timestamp + timestamp_inter) - current_timestamp
            else:
                alfa = lenght_closest_point/lenght_best_lap
                timestamp_inter = (third_nearest_point.timestmp - nearest_point.timestamp) * alfa
                absolute_delta = (nearest_point.timestamp + timestamp_inter) - current_timestamp
        ...
        if self.first_point:
            ...
            self.data = calc_distance(self, x, y, timestamp)
