...
def find_racebox(filepath):
    ...
    var_gyro = calc_var_gyro(filepath + 'RB-1.csv', 'racebox')
    var_acc = calc_var_acc(filepath + 'RB-1.csv', 'racebox')
    var_gps = calc_var_gps(filepath + 'RB-1.csv', 'racebox')
...
def open_file(filefullpath):
    with open(filefullpath, 'r') as file:
        ...
        for row in data:
            ...
            f_acc = kalman_acc(f_acc, GForceX, GForceY, GForceZ, f_gps[0].x[1][0], f_gps[1].x[1][0], row_counter)
