...
def import_acc(filefullpath):
    ...
    for row in data:
        ...
        #state estimate
        x_hat = np.array([[phi_hat_rad], [theta_hat_rad]])
        ...
        # MEASUREMENT UPDATE
        measurement = np.array(row[2], row[3])
        innovation = measurement - H @ x_hat[:2]
        x_hat_ = x_hat + K @ np.concatenate((innovation, np.zeros(2)))

        fused_states.append(x_hat_[:2])
                            
        point = (
          Point('gps')
          .tag("ID", "gps_position")
          .field("latitude", fused_states[0])
          .time(time)
        )
        points.append(point)
        point = (
          Point('gps')
          .tag("ID", "gps_position")
          .field("longitude", fused_states[1])
          .time(time)
        )
        points.append(point)
